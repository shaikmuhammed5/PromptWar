# Thunai (துணை) — Recovery & Prevention Companion

**Thunai** = Tamil for *the companion who stands beside you*.
...
Why the name matters to the product: this is not a clinic app, it is a companion. Every screen speaks in
...
1. Landing: name **Thunai**, tagline in Tamil + English, two big buttons — **"I am recovering"** / **"I am a caregiver"**.
...
6. Under the script, three always-present buttons: **Call [Thunai name]**, **Breathe with me** (4-7-8 animated timer), **Tele-MANAS 14416**.
...
4. UI renders the recommended safety tools **contextually** — high risk surfaces SOS + call Thunai; low risk surfaces journaling + a lesson.
...
- **Gemini 2.5 Flash** via `@google/genai`, server-side route handlers only (key never reaches client) — text, vision, streaming
...
- **Google Gemini 2.5 Flash** — SOS de-escalation scripts (streaming), voice check-in analysis (structured JSON), refusal scripts, lessons + quizzes, caregiver guidance. Location: `src/lib/ai/*` and all routes under `src/app/api/ai/`.
- **Gemini 2.5 Flash Vision** (image parts, same model) — trigger journal photo analysis. Location: `src/app/api/ai/journal/route.ts`.
